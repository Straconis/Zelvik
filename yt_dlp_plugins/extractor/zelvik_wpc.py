...
async def get_webpo_client_path(tab, logger):
    ...
    while (
        count < 10
        and not await tab.evaluate(
            f"!!{webpo_client_path}"
        )
    ):
        logger.debug(
            "Waiting for WebPoClient to be available "
            "in browser..."
        )

        # check that ytcfg is loaded and
        # bg_st_hr experiment is enabled
        if not await tab.evaluate(
            "!window.top['ytcfg']?.get('EXPERIMENT_FLAGS') "
            "|| !!ytcfg.get('EXPERIMENT_FLAGS')?.bg_st_hr"
        ):
            logger.warning(
                "bg_st_hr experiment is not enabled, "
                "WebPoClient may not be available.",
                once=True,
            )

        await asyncio.sleep(
            WEB_PO_BACKOFF_SECONDS
        )
...
async def mint_po_token(
    ...
    mint_po_token_code = f"""
        {webpo_client_path}().then(
            (client) => client.mws({json.dumps(mws_params)})
        ).catch(
            (e) => {{
    ...
    while tries < 10:
        ...
        logger.debug(
            "Waiting for WebPoClient to be ready "
            "in browser..."
        )
    ...
    raise PoTokenProviderError(
        "Timed out waiting for WebPoClient "
        "to be ready in browser"
    )


async def launch_browser(config):
    # todo: allow to specify an existing
    ...
    # Keep the headed browser minimized after startup.
    # Chrome is also configured to launch far off-screen
    # so the user should never see the initial window flash.
    try:
        window_id, _ = (
            await browser.main_tab.get_window()
        )

        await browser.main_tab.send(
            nodriver.cdp.browser.set_window_bounds(
                window_id=window_id,
                bounds=nodriver.cdp.browser.Bounds(
                    window_state=(
        ...
        # Failing to minimize should not prevent
        # PO-token generation.
        pass
...
class WPCPTP(PoTokenProvider):
    ...
    BUG_REPORT_LOCATION = (
        "https://github.com/"
        "coletdjnz/yt-dlp-getpot-wpc/issues"
    )
    ...
    def get_nodriver_config(
        ...
        # IMPORTANT:
        #
        # Do NOT use headless=True here.
        #
        # We proved that YouTube accepts WPC tokens
        # generated by headed Chrome, but rejects
        # tokens generated in headless mode.
        #
        # Instead we keep Chrome fully headed while
        # launching its window far outside the visible
        # desktop and minimized.
        browser_args = [
    ...
    def is_available(self):
        ...
        try:
            ...
            if "chrome" in str(e):
                ...
                self.logger.warning(
                    ...
                    "browser config: "
                    f"{e}"
                    f"{provider_bug_report_message(self)}"
                )
        ...
        if (
            ...
            or not pathlib.Path(
                nodriver_config.browser_executable_path
            ).exists()
        ):
            self.logger.debug(
                "WPC PO Token Provider requires Chrome "
                "to be installed. You can specify a path "
                'to the browser with --extractor-args '
                '"youtubepot-wpc:browser_path=XYZ".'
            )
    ...
    def _real_request_pot(
        ...
        if (
            ...
            self.logger.info(
                "Launching youtube.com in browser "
                "to retrieve PO Token(s). "
                "This will stay open while yt-dlp "
                "is running. Do not close the browser "
                "window!"
            )
        ...
        self.logger.info(
            f"Minting {request.context.value} "
            f"PO Token for "
            f"{request.internal_client_name} "
            "using WebPoClient in browser"
        )
